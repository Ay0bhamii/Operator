...
function App() {
  ...
  return <main className="site">
    ...
    <section id="lab" className="lab-section"><div className="section-heading"><div><span>02</span><h2>CHALLENGES</h2></div><p>5 VERIFIED<br/>COMPETITIVE GAMES</p></div><div className="game-list">{games.filter(g=>rankedGames.includes(g.id)).map((g,i)=>{const Icon=g.icon;return <button className="editorial-game" key={g.id} onClick={()=>{triggerFeedback("tap"); launchGame(g.id)}}><span>0{i+1}</span><Icon size={20}/><div><b>{g.name}</b><small>{g.subtitle}</small></div><small>RANKED</small><strong>-&gt;</strong></button>})}</div></section>
