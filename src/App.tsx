...
function App() {
  ...
  return <main className="site command-site">
    ...
    <section className="hero-editorial operations-hero arcade-hero"><div className="hero-copy"><div className="kicker"><span/>FEATURED GAME · VERIFIED SKILL RUN</div><div className="hero-badge"><Sparkles size={14}/> PLAY. COMPETE. EARN.</div><h1>{dashboardDaily?.completed ? "MISSION\nCOMPLETE" : "LEVEL UP\nYOUR"}<br/><em>{dailyGame?.name.toUpperCase() ?? "NIM PIN"}</em></h1><p>{dashboardDaily?.completed ? "Your score is on the board. Improve it only if the daily rules permit another attempt." : wallet ? "Your daily skill test is ready. Make the run count." : "Enter the Nimiq Arcade. Play free, then connect to take your place on the board."}</p><div className="hero-actions"><button className="gold-btn" disabled={!dailyOperation} onClick={()=>dailyOperation && (wallet ? requestRankedGame(dailyOperation.gameId as GameId,"daily") : launchGame(dailyOperation.gameId as GameId,"practice"))}><Play size={15} fill="currentColor"/> {dashboardDaily?.completed ? "PLAY AGAIN" : "PLAY NOW"}</button></div></div><div className="operation-console arcade-console"><div className="console-art"><Gamepad2 size={62}/><span>DAILY DROP</span></div><span>ENDS IN</span><b>{formatCountdown(dailyRemaining)}</b><div><small>YOUR BEST</small><strong>{dashboardDaily?.score?.toLocaleString() || "—"}</strong></div><div><small>DAILY RANK</small><strong>{dashboardDaily?.rank ? `#${dashboardDaily.rank}` : "—"}</strong></div></div></section>
