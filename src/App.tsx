...
function App() {
  ...
  const [profile,setProfile]=useState<{username:string|null;rating:number;grade:string;displayGrade?:string;verifiedRuns:number;streak:number}|null>(null);
  ...
  async function finish(id:GameId,r:Result){
    if(activeRun){
      ...
      try {
        ...
        setRankedSubmission({state:"verified",score:result.score,xp:result.xp,completed:"completed" in result ? result.completed : true,rank:result.rank,previousBest:result.previousBest,improvement:result.improvement,personalBest:result.personalBest,ratingDelta:result.ratingDelta,rating:result.rating,grade:result.grade,nextGrade:result.nextGrade,ratingToNext:result.ratingToNext,progressPercent:result.progressPercent,playerUsername:profile?.username || "You",...(activeRun.challengeToken && "opponentScore" in result ? {opponentUsername:result.opponentUsername,opponentScore:result.opponentScore,winnerUsername:result.winnerUsername} : {})});
  ...
  return <main className="site command-site">
    <header className="nav command-nav"><button className="wordmark" onClick={()=>{setView("operations");scrollTo(0,0)}}><img className="brand-logo" src="/logo/operator-mark.svg" alt=""/><span className="wordmark-main">OPERATOR</span><span className="wordmark-sub">BY NIMIQ</span></button><nav className="nav-links command-links"><button className={view==="operations" ? "active" : ""} onClick={()=>setView("operations")}>OPERATIONS</button><button className={view==="rankings" ? "active" : ""} onClick={()=>setView("rankings")}>RANKINGS</button><button className={view==="profile" ? "active" : ""} onClick={()=>setView("profile")}>PROFILE</button></nav>{wallet ? <button className="connect operator-connect" onClick={signOut}><i/>{profile?.username || "OPERATOR"}</button> : <button className="connect" onClick={connect}><i/>{isNimiqPay()?"Connect Nimiq Pay":"PLAY AS GUEST"}</button>}</header>
    ...
    {view === "operations" && <>
    <section className={`operator-status ${dashboardLoading ? "is-loading" : ""}`}><div><span className="eyebrow">OPERATOR STATUS</span><h1>{operator?.username || profile?.username || "GUEST OPERATOR"}</h1><p>{dashboardLoading ? "SYNCING VERIFIED DATA" : operator?.displayGrade || profile?.displayGrade || "UNRANKED"}</p></div><div className="status-stat"><small>GLOBAL RANK</small><b>{operator?.globalRank ? `#${operator.globalRank}` : "—"}</b></div><div className="status-stat"><small>RATING</small><b>{operator?.rating?.toLocaleString() || "—"}</b></div><div className="status-stat"><small>LEVEL / XP</small><b>{operator ? `${operator.level} / ${operator.xp.toLocaleString()}` : "—"}</b></div><div className="status-stat"><small>STREAK</small><b>{operator?.streak ? `🔥 ${operator.streak} DAYS` : "—"}</b></div></section>
    <section className="hero-editorial operations-hero"><div className="hero-copy"><div className="kicker"><span/>TODAY'S OPERATION · VERIFIED SKILL RUN</div><h1>{dashboardDaily?.completed ? "MISSION\nCOMPLETE" : "TODAY'S\nOPERATION"}<br/><em>{dailyGame?.name.toUpperCase() ?? "NIM PIN"}</em></h1><p>{dashboardDaily?.completed ? "Your score is on the board. Improve it only if the daily rules permit another attempt." : wallet ? "Your daily skill test is ready. Make the run count." : "Practice is open to everyone. Sign in when you are ready for a verified daily result."}</p><div className="hero-actions"><button className="gold-btn" disabled={!dailyOperation} onClick={()=>dailyOperation && (wallet ? requestRankedGame(dailyOperation.gameId as GameId,"daily") : launchGame(dailyOperation.gameId as GameId,"practice"))}>{dashboardDaily?.completed ? "PLAY AGAIN" : wallet ? "PLAY NOW" : "PRACTICE NOW"} <b>-&gt;</b></button></div></div><div className="operation-console"><span>ENDS IN</span><b>{formatCountdown(dailyRemaining)}</b><div><small>YOUR BEST</small><strong>{dashboardDaily?.score?.toLocaleString() || "—"}</strong></div><div><small>DAILY RANK</small><strong>{dashboardDaily?.rank ? `#${dashboardDaily.rank}` : "—"}</strong></div><p>{dashboardDaily?.pointsToNext ? `${dashboardDaily.pointsToNext} points to the player above.` : "Make your first verified daily run."}</p></div></section>
...
function RankedResult({submission,preview,onRestart}:{submission:RankedSubmission;preview:Result;onRestart:()=>void}) {
  ...
  if (submission.state === "verified") {
    ...
    return <div className="result"><div className="result-icon"><Trophy/></div><small>{challengeComplete ? "CHALLENGE COMPLETE" : submission.completed === false ? "VERIFIED PARTIAL RESULT" : "VERIFIED RESULT"}</small>{challengeComplete ? <div className="challenge-result"><div><span>@{submission.playerUsername}</span><b>{submission.score?.toLocaleString()}</b></div><div><span>@{submission.opponentUsername || "Opponent"}</span><b>{(submission.opponentScore ?? 0).toLocaleString()}</b></div><strong>{outcome}</strong></div> : <><h2>{submission.score?.toLocaleString()}</h2>{submission.completed === false && <p className="result-callout">RUN ENDED — VERIFIED PROGRESS SAVED</p>}{submission.personalBest ? <><p className="result-callout">NEW PERSONAL BEST {submission.improvement && submission.improvement > 0 ? `+${submission.improvement} points` : ""}</p><div className="best-comparison"><span>PREVIOUS: {(submission.previousBest ?? 0).toLocaleString()}</span><b>NEW: {(submission.score ?? 0).toLocaleString()}</b></div></> : <p>BEST: {(submission.previousBest ?? submission.score ?? 0).toLocaleString()} - {Math.abs(submission.improvement || 0)} points to improve</p>}<p>+{submission.xp} XP {submission.ratingDelta ? `- ${submission.ratingDelta > 0 ? "+" : ""}${submission.ratingDelta} RATING` : ""}</p><p>{submission.rank ? `GLOBAL RANK #${submission.rank}` : "RANKING UPDATED"} {submission.nextGrade && submission.ratingToNext ? `- ${submission.ratingToNext} TO ${submission.nextGrade}` : ""}</p></>}<button className="primary" onClick={isChallenge ? ()=>{window.location.href="/#ranked"} : onRestart}><RotateCcw size={16}/> {isChallenge ? "New challenge" : submission.personalBest ? "Beat your best" : "Run again"}</button></div>;
  }
...
function NimPin({seed,rankedSubmission,onEvent,onFinish}:{seed?:string;rankedSubmission:RankedSubmission;onEvent:(event:Omit<GameEvent,"t">)=>void;onFinish:(r:Result)=>void}) {
  ...
  function key(k:string){if(done||show)return; const n=input+k;if(n.length<=pin.length){onEvent({type:"key",value:k});setInput(n)} if(n.length===pin.length){if(n===pin){const score=Math.max(100,Math.round(time*100));setDone(true);onFinish({score,xp:125,time:12-time})}else{setDone(true);onFinish({score:0,xp:25})}}}
...
function Sync({ranked,rankedSubmission,onEvent,onFinish}:{ranked:boolean;rankedSubmission:RankedSubmission;onEvent:(event:Omit<GameEvent,"t">)=>void;onFinish:(r:Result)=>void}) {
  const [pos,setPos]=useState(0); const [dir,setDir]=useState(1); const [tries,setTries]=useState(3); const [time,setTime]=useState(30); const [done,setDone]=useState(false); const [score,setScore]=useState(0);
  ...
  useEffect(()=>{if(done)return;const timer=setInterval(()=>setTime(value=>{if(value<=1){setDone(true);onFinish({score,xp:0,time:30});return 0}return value-1}),1000);return()=>clearInterval(timer)},[done,onFinish,score]);
  ...
  return <div className="challenge"><GameHUD label="SYNC" value={`${score} PTS`} timer={`${tries} attempts · ${time}s`}/><div className="sync-track"><div className="sync-target"/><div className="sync-cursor" style={{left:`${pos}%`}}/></div><button className="primary huge" onClick={hit}>SYNC PACKET</button></div>
}
