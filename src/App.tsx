...
function App() {
  ...
  async function connect(){
    ...
    try {
      ...
      if(a){
        ...
        getMe().then(profile=>{setXp(profile.xp);setProfile(profile)}).catch(()=>{});
      }
    } catch (e) {
  ...
  async function launchGame(id: GameId, mode: "ranked" | "daily" = "ranked") {
    ...
    if (wallet && rankedGames.includes(id)) {
      setActiveRun(await startRun(id, rankedMode));
  ...
  return <main className="site">
    ...
    <section className="hero-editorial"><div className="hero-copy"><div className="kicker"><span/>DAILY OPERATION · VERIFIED SKILL RUN</div><h1>DAILY<br/><em>{dailyGame?.name.toUpperCase() ?? "NIM PIN"}</em></h1><p>Complete today’s challenge and climb the day’s leaderboard before the timer resets.</p><div className="hero-actions"><button className="gold-btn" disabled={!dailyOperation} onClick={()=>dailyOperation && (wallet ? launchGame(dailyOperation.gameId as GameId,"daily") : connect())}>{wallet?"PLAY DAILY":"CONNECT TO PLAY"} <b>?</b></button><a className="text-btn" href="#feature">VIEW OPERATION ?</a></div></div><div className="hero-emblem"><div className="orbit a"/><div className="orbit b"/><div className="core"><img src="/logo/operator-mark.svg" alt="OPERATOR"/></div><small>DAILY / 01</small></div></section>
    ...
    <section id="feature" className="feature-section"><div className="section-label">01 <span>DAILY OPERATION</span></div><div className="feature-card daily-card"><div className="daily-content"><small>TODAY'S CHALLENGE</small><h2>{dailyGame?.name.toUpperCase() ?? "NIM PIN"}</h2><div className="daily-countdown">? {formatCountdown(dailyRemaining)} remaining</div><div className="daily-meta"><span>1 attempt</span><span>Your rank: —</span></div><button className="gold-btn compact" onClick={()=>{triggerFeedback("tap"); wallet && dailyOperation ? launchGame(dailyOperation.gameId as GameId, "daily") : connect();}}>{wallet ? "PLAY" : "CONNECT"}</button></div></div></section>
    <section id="lab" className="lab-section"><div className="section-heading"><div><span>02</span><h2>CHALLENGES</h2></div><p>5 VERIFIED<br/>COMPETITIVE GAMES</p></div><div className="game-list">{games.slice(0,5).map((g,i)=>{const Icon=g.icon;return <button className="editorial-game" key={g.id} onClick={()=>{triggerFeedback("tap"); launchGame(g.id)}}><span>0{i+1}</span><Icon size={20}/><div><b>{g.name}</b><small>{g.subtitle}</small></div><small>RANKED</small><strong>?</strong></button>})}</div></section>
